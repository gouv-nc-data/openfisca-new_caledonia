...
These elements are described in different folders. All the modelling happens within the `openfisca_new_caledonia` folder.
...
```sh
pip install openfisca-new_caledonia
```
...
```sh
git clone git://github.com/gouv-nc-data/openfisca-new_caledonia.git.git
cd openfisca-new_caledonia.git
...
```sh
openfisca serve --port 5000 --country-package openfisca_new_caledonia
```
...
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_new_caledonia/situation_examples/couple.json \
